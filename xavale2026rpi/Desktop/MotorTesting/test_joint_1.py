...
async def main():
    ...
    state = await controller.set_stop(query=True)  # or use set_position with query=True
    starting_position = state.values[moteus.Register.POSITION]
    starting_velocity = state.values[moteus.Register.VELOCITY]
    ...
    try:
        ...
        for i in range(200):
            ...
            #     velocity=starting_velocity + math.sin(N*2*math.pi*i/200),
            #     position=math.nan,      
            #     maximum_torque=1.0, 
            #     query=True
            # )
